...
class EmailNotifier:
    ...
    def send_invitation(
        ...
        *,
        ...
        html = (
            f"<p>{inviter_email} has invited you to join <strong>{organization_name}</strong> "
            f"on ARIE as <strong>{role}</strong>.</p>"
    ...
    def send_review_required(
        ...
        *,
        ...
        html = (
            f"<p>A lead needs human review in <strong>{organization_name}</strong>.</p>"
            f"<p>{summary}</p>"
            f'<p><a href="{review_url}">Review it</a></p>'
        )
        return self._sender.send(
            EmailMessage(
                to_email=to_email,
    ...
    def send_usage_warning(
        ...
        *,
        ...
        html = (
            f"<p><strong>{organization_name}</strong> has used {used:.2g} of {limit:.2g} "
            f"{metric_label} ({percent:.0%}) this billing period.</p>"
            f'<p><a href="{billing_url}">Manage your plan</a></p>'
        )
        return self._sender.send(
            EmailMessage(
                to_email=to_email,
    ...
    def send_limit_reached(
        ...
        html = (
            f"<p><strong>{organization_name}</strong> has reached its {metric_label} limit for "
            "this billing period.</p>"
            f'<p><a href="{billing_url}">Upgrade your plan to continue</a></p>'
        )
        return self._sender.send(
            EmailMessage(
                to_email=to_email,
    ...
    def send_payment_problem(
        ...
        html = (
            f"<p>There was a problem with <strong>{organization_name}</strong>'s subscription "
            f"payment: {reason}.</p>"
            f'<p><a href="{portal_url}">Update your payment method</a></p>'
        )
        return self._sender.send(
            EmailMessage(
                to_email=to_email,
