...
- **`arie-api`** — Dockerfile build, no start-command override needed:
  ...
  Railway:
  `https://arie-b2b-enrichment-engine-production.up.railway.app`
  (the hostname tracks the repository name, not the service name). The
  Railway *project* holding these services was replaced during
  Productization M2C and the original one retired in M5; as of the M6
  rollout it is `discerning-essence`, environment `production`. That name is
  a fact about an account, not about this repository, so confirm it against
  the Railway dashboard before relying on it — the hostname above is the
  value that actually has to be right, since Stripe's webhook endpoint and
  the frontend's `NEXT_PUBLIC_ARIE_API_BASE_URL` both point at it.
  Healthcheck
