...
- **A specific hosting platform.** Picking one, provisioning it, and wiring
  its secret manager is out of scope here — everything above is written to
  transfer to whichever one gets chosen.
...
- **`PROVIDER_MODE=live`.** No real enrichment provider adapter exists yet
  (ADR 0003); deploying with this set is deploying something that refuses to
  start, by design.
